...
# In interpolation search we find the desired item on the first attempt
# It works well when the data set is sorted, and uniformly distributed

# Compute the midpoint
def nearest_mid(input_list, low_index, upper_index, search_value):
    mid = low_index + ((upper_index - low_index) / (
        input_list[upper_index] - input_list[low_index])
...
def interpolation_search(ordered_list, search_value):
    ...
    while low_index <= upper_index:
        mid_point = nearest_mid(ordered_list, low_index,
                                upper_index, search_value)
...
da = interpolation_search(list1, 120)
print("Index position of value 120 is ", da)
